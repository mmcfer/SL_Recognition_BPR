function   hand_posture=extract_data_hand_depth_no_binary_close_object(folder,folder2) 
...
%        for m=1:size(depth_hand_2,2)
%                   for n=1:size(depth_hand_2,2)
%               im_depth_x(m,n)=(im_depth_x(m,n)./524).*(n-320);
%               im_depth_y(m,n)=(im_depth_y(m,n)./524).*(m-240);  
...
result = find_hand_marc_rot(depth_hand_2);
    depth_hand_2= depth_hand_2(:,result(2):result(1));
        bin= bin(:,result(2):result(1));
depth_hand_2=depth_hand_2.*bin;

CC = bwconncomp(depth_hand_2);
 numPixels = cellfun(@numel,CC.PixelIdxList);
[biggest,idx] = max(numPixels);
STATS = regionprops(CC, 'BoundingBox');
BoundingBox=floor(cat(1,STATS.BoundingBox));
